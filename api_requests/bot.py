...
async def city_chosen(message: types.Message, state: FSMContext):
    ...
    if data:
        ...
        await message.answer("Такого города не существует", reply_markup=markup)
        await state.finish()
...
async def set_user_city(message: types.Message):
    ...
    text = 'Введите название города'
    await message.answer(text, reply_markup=markup)
    await ChoiceCityWeather.user_city.set()        

@dp.message_handler(state=ChoiceCityWeather.user_city)
async def set_city(message: types.Message, state: FSMContext):
    ...
    city = await state.get_data()
    orm.set_user_city(message.from_user.id, city.get('user_city'))
    markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2)
    btn1 = types.KeyboardButton('Погода в моём городе')
    btn2 = types.KeyboardButton('Погода в другом месте')
    btn3 = types.KeyboardButton('История')
    ...
    print(f'Ваш город: {city.get("user_city")}')
    await message.answer(f'Ваш город: {city.get("user_city")}', reply_markup=markup)
    await state.finish()
...
async def get_user_city_weather(message: types.Message):
    ...
    if city is None:
        await message.answer("Установите свой город", reply_markup=markup)
        markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
